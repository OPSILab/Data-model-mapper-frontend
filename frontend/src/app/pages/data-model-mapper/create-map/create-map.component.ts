...
export class CreateMapComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loaded = false;
    if (this.value)
      //TODO check if it is required using value
      for (const key in this.value) this[key] = this.value[key];
    console.debug(this);
    console.debug(this.saveSchema);
    console.debug(this.saveSource);
    if (!this.unsaved.schema) this.saveSchema = false;
    if (!this.unsaved.source) this.saveSource = false;
    console.debug(this.saveSchema);
    console.debug(this.saveSource);
  }
  ...
  selectFiltered(property: string, key: string, value: string) {
    try {
      return this[property].filter(
        (filteredProperty: { [x: string]: string | object | number; mapRef: string }) => filteredProperty[key] == value && filteredProperty.mapRef
      )[0];
    } catch (error) {
      console.error(error);
      return false;
  ...
  async submit() {
    ...
    if (this.sourceNameAlreadyExists())
      //TODO this way a different source with the same name will be overwritten. Change this or sourceNameAlreadyExists fn
    ...
    if (!errors && this.schemaNameAlreadyExists())
      //TODO this way a different schema with the same name will be overwritten. Change this or schemaAlreadyExists fn
    ...
    if (!errors)
      try {
        ...
        if (this.save) {
          ...
          console.debug(this.mapperRecordId)
          if (!errors && this.saveSchema)
            try {
              console.debug(this.schemaSaved)
              if (!this.schemaSaved) await this.dmmService.saveSchema(name, this.mapperRecordId, status, description, this.schema);
              else await this.dmmService.updateSchema(name, this.mapperRecordId, status, description, this.schema);
            } catch (error) {
          ...
          if (!errors && this.saveSource)
            try {
              if (!this.sourceSaved)
                await this.dmmService.saveSource(
                  name,
                  this.mapperRecordId,
                  status,
                  description,
                ...
                await this.dmmService.updateSource(
                  name,
                  this.mapperRecordId,
                  status,
                  description,
          ...
          mapperRecord.mapperRecordId = this.mapperRecordId = mapperRecord._id;
          mapperRecord.description = this.description;
          mapperRecord.status = this.status;
          if (!errors && this.saveSchema)
            try {
              if (!this.schemaSaved) await this.dmmService.saveSchema(name, this.mapperRecordId, status, description, this.schema);
              else await this.dmmService.updateSchema(name, this.mapperRecordId, status, description, this.schema);
  ...
  errorHandle(entity: string, error) {
    ...
    if (!this.schema && !this.dataModelID && !this.dataModelURL)
      errors.push({
        path: 'root.schema',
        message: 'Value required',
        errorcount: JSON.parse(JSON.stringify(errors.length)) + 1,
      });
    if (!this.mapperRecordId) console.error('could not retrieve mapperRecordId');
    if (!this.name)
      errors.push({
        path: 'root.name',
        property: 'minLength',
    ...
    if (!this.sourceData && !this.sourceDataURL && !this.sourceDataID && !this.bucket && !this.minioObjName)
      errors.push({
        path: 'root.source',
        property: 'minLength',
        message: 'Value required',
    ...
    if (error.status == 403 || error.status == 401) this.router.navigateByUrl('/login/loginPopup');
    else if (error.status == 0) this.errorService.openErrorDialog(error);
